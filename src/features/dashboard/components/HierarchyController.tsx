...
export const HierarchyController: React.FC<HierarchyControllerProps> = ({
  ...
  const sortedClusters = useMemo(() => {
    return sortClusters(Object.keys(clusterTeamMap));
  }, [clusterTeamMap]);

  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h3 className={styles.headerTitle}>{formatDisplayText('Cluster / Team')}</h3>
      </div>
          ...
          return (
            <div key={clusterName} className={styles.clusterGroup}>
              ...
              >
                ...
                <span className={styles.clusterName}>{formatDisplayText(clusterName)}</span>
                  ...
                  >
                    {teamsInCluster.map(teamName => {
                      return (
                        ...
                        >
                          <span className={styles.teamName}>{formatDisplayText(teamName)}</span>
