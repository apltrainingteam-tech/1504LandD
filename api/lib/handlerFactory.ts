...
import { dbConnect, getModel } from './db';
...
export async function baseHandler(
  ...
  try {
    console.log(`[API] ${req.method} /api/${collectionName} - Initializing...`);
    
    // 1. Connect to DB
    await dbConnect();
    const dbName = mongoose.connection.db?.databaseName || 'unknown';
    
    // 2. Get Model (Initialized inside the handler)
    const Model = getModel(modelName, collectionName) as any;
    
    const { id, action, field, value, values, clear, clearByField, batch, upsert, query: bodyQuery } = { ...req.query, ...req.body } as any;

    console.log(`[API] Connected to DB: ${dbName}. Requesting: ${collectionName}`);
    ...
    console.error(`[API ERROR] /api/${collectionName}:`, error.message);
